...
\Abstract{ \R package \ls \citep{Soetaert08b} solves linear inverse models (LIM), consisting
...
Solve(E,F)
@
In the next example, an additional equation, which is a linear comination of the first two
is added to the model (i.e. $eq_4=eq_1+eq_2$). This problem is therefore equivalent to the previous one;
...
$\min_{x}\| \mathbf{A} \cdot \mathbf{x} - \mathbf{b} \|^2$.

The least squares solution can be singled out by function \emph{lsei} (\emph{l}east \emph{s}quares
...
the equalities that have to be met exactly and of the violated inequalities. As in this
...
It is also possible to merge equalities that have to be matched exactly with equations that
have to be approximated as closely as possible:
<<over2 with ineq>>=
E <- c(0,1,0)
F <- 3
...
@
Note that for the case above, it is also possible to perform Bayesian sampling,
using \ls function \code{xsample} (see below).

\section{Undetermined systems}
...
Functions \code{Solve} and \code{ldei} retrieve the parsimonious solution.
...
@
In this particular case, it is also possible to randomly sample the solution space.
...
The next example also includes inequalities:
...
As before, the parsimonious solution can be found by functions \code{ldei} and \code{lsei}.
...
H <- c(-1, 2, 1)
ldei(E,F,G=G,H=H)$X
(pars<-lsei(E,F,G=G,H=H,A=diag(nr=4),B=rep(0,4))$X)
@
We can also estimate the minimal and maximal values of all unknowns using function \code{xranges}.
...
Function \code{xsample} randomly samples the underdetermined problem.
For small problems, the coordinates directions algorithm ("cda") is a good choice.

We use \R function \code{pairs}, with a density function on the diagonal,
 to visualise the output (figure \ref{fig:u2}).
...
@
Function \code{xsample} randomly samples the underdetermined problem
(using the metropolis algorithm), selecting
 likely values given the approximate equations. The
...
(the shart edges) as well as by the approximate equations (figure \ref{fig:u3}).
...
In previous section, the parsimonious solution, i.e.
the solution for which the sum of squared values is minimal was selected.
...
In case the unknowns need not be positive, they are rewritten as the difference of
two positive unknowns as follows:
\[
x_i = {x_i}^a -{x_i}^b
\]
where both ${x_i}^a$ and ${x_i}^b$ are positive quantities, but where ${x_i}$ can be
negative (if ${x_i}^a<{x_i}^b$.

The linear programming problem is then rewritten as a function of these new unknowns
as follows:
...
GG <- cbind(G,-G)
CCost <- c(Cost, -Cost)
lp <-linp(E=EE,F=F,G=GG, H= H, Cost=CCost)
X <- lp$X[1:4]-lp$X[5:8]
X
lp$solutionNorm
@
...
\begin{itemize}
 ...
 Biologists may know this problem as "Chemtax" \citep{Mackey96}. See also \R-package \pkg{BCE} \citep {Meersche08},
  \citep {Meersche08b}.  The problem contains 8 unknowns; it consists of 1 equality, 12 approximate equations,
...
For solving linear programming, \code{lp} from \pkg{lpSolve} is the only algorithm included.
(It is the most robust linear programming algorithm we are familiar with). For quadratic
programming, we added the code \code{lsei}, which in our experience often gives a valid
...
Another \R-package, \pkg{LIM} (\citep{Soetaert08c})
\footnote{at this time, this package may not yet be on CRAN; it may be found on R-forge}
...
The various functions in \ls are given in table (1).
\begin{table*}[t]
