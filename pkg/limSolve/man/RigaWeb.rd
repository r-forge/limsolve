...
\docType{data}
\alias{RigaWeb}
\title{An underdetermined linear inverse problem: the Gulf of Riga *spring* planktonic food web}
\description{Input matrices and vectors for estimating the flows in the planktonic food web of the Gulf of Riga.

(as in Donali et al. (1999)).

The original input file can be found in the package subdirectory \url{/inst/docs/RigaSpring.input}

There are 7 functional compartments:
P1,P2,B,N,Z,D,OC
(two phytoplankton groups, Bacteria, Nanozooplankton, Zooplankton, Detritus and DOC).
...
P1->DOC, P1->SED, P2->DOC, P2->Z, P2->D, P2->SED, N->DOC, N->Z, Z->DOC, Z->D,
Z->SED, D->Z, D->DOC, D->SED, B->N, B->SED, DOC->B

The lsei model contains:
\item 14 equalities (Ax=B): the 7 mass balances (one for each compartment) and 7 measurement equations
\item 26 unknowns (x), the flow values
\item 45 inequalities (Gx>h). The first 19 inequalities impose bounds on some combinations of flows.
The last 26 inequalities impose that the flows have to be positive.

As there are more unknowns (26) than equations (14), there exist an infinite amount of solutions
...
The columnames of \code{A} and \code{G} are the names of the unknown reaction rates;
The first 14 rownames of \code{A} give the names of the components (these rows consitute the mass balance equations).
